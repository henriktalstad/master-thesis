#!/usr/bin/env bun
/** @see package.json script `backfill-control-signal-hours` */
export {};

/**
 * Backfill materialiserte time-buckets fra eksisterende pipeline-replay.
 *
 * Usage:
 *   bun run backfill-control-signal-hours
 *   bun run backfill-control-signal-hours -- sorgenfri
 */
async function main() {
  ...
  const { hoursWritten } = await upsertControlSignalHoursFromSteps({
    buildingId: building.id,
    steps,
    pipelineRunId: runId,
  });

  console.info(`Ferdig — ${hoursWritten} time-buckets skrevet.`);
}
